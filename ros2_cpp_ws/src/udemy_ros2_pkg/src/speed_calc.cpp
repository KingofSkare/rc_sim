...
class SpeedCalcNode : public rclcpp::Node
{
    ...
    private:
        void calculate_and_pub_speed(const std_msgs::msg::Float64 & rpm_msg) const
        {
            ...
            float wheel_radius_parameter=DEFAULT_WHEEL_RADIUS;
            this->get_parameter("wheel_radius", wheel_radius_parameter);

            // Speed [m/s] = RPM [rev/min] * wheel_Circumference [meters/rev] / 60 [secunds/min]
            speed_msg.data = rpm_msg.data * (2 * wheel_radius_parameter * M_PI) / 60;
